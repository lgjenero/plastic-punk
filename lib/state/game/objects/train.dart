...
class Train extends PositionComponent {
  ...
  static const double _speed = 8;
  static const double _size = 18;

  Train(TilePosition start, this.state, {super.priority}) : _tilePosition = start {
  ...
  FutureOr<void> onLoad() async {
    _topDownSprite = SpriteComponent(
      sprite: Sprite(Flame.images.fromCache('animations/train.png'),
          srcSize: Vector2.all(128), srcPosition: Vector2.zero()),
    ...
    _leftRigthSprite = SpriteComponent(
      sprite: Sprite(Flame.images.fromCache('animations/train.png'),
          srcSize: Vector2.all(128), srcPosition: Vector2(128, 0)),
      size: Vector2.all(_size),
      position: Vector2.all(_size / 2),
      anchor: Anchor.center,
    );
  ...
  void _initialChooseDirection() {
    ...
    for (final neighbour in neighbours) {
      ...
      if (AppTiles.transportationTiles.contains(buildingId)) {
